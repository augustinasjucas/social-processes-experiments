...
def plot_batch(context: Seq2SeqSamples, target:Seq2SeqSamples,
    ...
    fig, axs = plt.subplots(height_cnt, col_cnt, figsize=(7.5*2, 12/3*height_cnt), sharey="row")
    ...
    if q_context is not None:
        ...
        pad = 10
        axs[3, 0].annotate(text="Prior ($q(z|C)$)", xy=(0, 0.5), xytext=(-axs[3, 0].yaxis.labelpad - pad, 0),
...
def plot_z_analysis (
    ...
    for i in range(meta_sample_cnt):
        for j in range(z_sample_count):
            ...
            #     range_for_predictions,
            #     mean - std, mean + std,
            #     alpha=0.35, facecolor=pred_style["color"], interpolate=True, label="predicted std. dev."
            # )
...
def plot_z_analysis_on_a_single(
    ...
    legend = fig.legend(handles, labels, loc="lower left", bbox_to_anchor=(0.3925, -0.1477),
